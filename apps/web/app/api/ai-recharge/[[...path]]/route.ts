...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyAiRechargeRequest(request, context);
}
...
async function proxyAiRechargeRequest(request: NextRequest, context: RouteContext) {
  const { path = [] } = await context.params;
  const targetPath = path.length > 0 ? `/${path.map((segment) => encodeURIComponent(segment)).join('/')}` : '';
  const targetUrl = `${INTERNAL_API_BASE_URL}/ai-recharge${targetPath}${request.nextUrl.search}`;
