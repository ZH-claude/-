...
建议下一次从 T12 开始：费用说明页面。

T12 的边界：

- 基于现有 `model_prices`、`user_groups`、`model_group_accesses` 和当前用户真实可用模型，新增费用说明页面。
- 用户只能看到自己分组可用模型、公开价格、模型倍率和分组倍率，不展示上游供应商密钥或内部价格快照。
- 首期不做支付、发票、复杂套餐和商用营销页，只做可搜索、可复制、可解释的真实价格表。

T12 完成指标：

- 用户可查看自己真实可用模型的输入/输出单价、模型倍率、分组倍率和计费公式说明。
- 用户可搜索模型并复制模型名。
- 页面数据来自后端真实模型配置，不使用硬编码假模型。
- 未登录用户不能访问个人化价格数据。
