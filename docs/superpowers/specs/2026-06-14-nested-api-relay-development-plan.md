...
- 已完成本地浏览器真实登录验证：`merchant_test_1 / MERCHANT_TEST_PASSWORD` 在 1920、1366、390 三个视口下商家 Shell 存在、普通用户菜单未泄漏、无横向溢出、控制台无错误。
...
- 已完成本地浏览器真实登录验证：`merchant_test_1 / MERCHANT_TEST_PASSWORD` 在 1920、1366、390 三个视口下 Dashboard 存在、普通用户菜单未泄漏、无横向溢出、控制台无错误。
...
- M06 商家端模型与上游配置页已完成：新增 `/merchant/model-config` 独立页面，商家端“上游/模型”菜单进入该页；页面使用真实后端和数据库管理分组、模型价格、上游配置、上游模型映射和健康检查。
- 已新增 `npm run qa:t22:merchant-model-config`，通过真实 Postgres 临时创建后台账号、普通用户、分组、模型、上游、映射和客户密钥，验证用户端价格页、账户可用模型、`/v1/models` 与商家配置互通；同时验证普通用户不能操作商家配置、上游密钥不明文外泄、密钥加密保存、本机/内网/metadata 健康检查被拦截；结束后临时数据残留为 0。
- 已完成本地浏览器真实验证：商家账号进入 `/merchant/model-config` 后，固定顶部栏和左侧栏存在，当前菜单高亮，桌面和手机宽度无横向溢出，页面无报错；登录页、注册页、首页、用户端与商家端公共品牌已由 `Relay Console` 改为中文“中转站控制台”。
- 已明确商用边界：当前 MVP 是单平台老板模式，即平台商家后台统一配置上游，客户使用本平台发放的密钥访问；多商家各自绑定独立上游密钥、客户按商家归属选择上游，已写入 `docs/product/merchant-console-plan.md` 的商用升级计划，不在 M06 中临时硬改。
- M07 商家端公告与审计页已完成：新增 `/merchant/announcements` 和 `/merchant/audit` 独立页面，商家端“公告”和“审计”菜单进入独立页面，旧 `/admin` 大页面保留兼容。
- 已新增 `npm run qa:t22:merchant-announcements-audit`，通过真实 Postgres 临时创建后台账号和普通用户，真实登录后验证公告发布、草稿/归档隐藏、用户公开首页只显示已发布公告、普通用户禁止访问后台公告和审计、审计分页和脱敏；结束后用户、钱包、会话、公告、后台审计和安全审计残留为 0。
- 已完成本地浏览器真实验证：商家账号进入公告和审计页面后，固定顶部栏和左侧栏存在，当前菜单高亮，桌面和手机宽度无横向溢出，页面无报错，没有出现旧英文品牌或敏感内容。
- M08 商家端服务状态与日志入口已完成：新增 `/merchant/service-status`、`/merchant/request-logs`、`/merchant/drawing-logs` 独立页面，商家端固定栏中的请求日志、绘图日志和服务状态进入真实商家页面。
- 已删除商家端里没有实际上游支撑的“异步任务”独立入口；绘图日志只读取真实 `kind=image` 记录，通用异步任务不会出现在绘图页。
- 已新增 `npm run qa:t22:merchant-service-logs`，通过真实 Postgres 临时创建后台账号、普通用户、令牌、请求日志、图片任务和通用任务，验证商家日志真实读取、普通用户禁止访问商家日志、绘图日志只显示图片任务、用户端日志和服务状态仍兼容；结束后临时用户、钱包、会话、令牌、请求日志和任务残留均为 0。
- 已完成本地 Chrome 真实登录验证：`merchant_test_1 / MERCHANT_TEST_PASSWORD` 在桌面 1366 宽度和手机 390 宽度下进入服务状态、请求日志和绘图日志三页，固定顶部栏和左侧栏存在，无“异步任务”误导入口、无 500、无旧英文品牌、无横向溢出，并保存截图证据。
- M09 角色隔离与端到端回归已完成：新增 `npm run qa:t22:merchant-role-isolation`，通过真实 Postgres 临时创建普通用户和商家账号，真实登录后验证用户端、商家端、会话、钱包和数据库一致性；结束后临时用户、钱包、会话和安全审计残留为 0。
- 已验证普通用户不能访问任一商家端页面或后台能力；未登录为 401 或跳登录页，普通用户为 403 或跳账户中心，商家账号可访问全部商家端核心页面和能力。
- 已验证用户端核心路径仍可用：账户、充值、令牌、日志、价格、模型、通知、绘图、服务状态；也验证商家端核心路径仍可用：用户、充值码、上游/模型、公告、审计、服务状态、请求日志、绘图日志。
- 本轮发现并修复无效上游健康检查返回 500 的问题，补齐无效编号校验；同时把旧 `/admin` 兼容页改为服务端角色保护，普通用户不能看到商家页壳。
- 已完成本地 Chrome 真实浏览器回归：临时普通用户真实注册登录，商家账号 `merchant_test_1 / MERCHANT_TEST_PASSWORD` 真实登录；桌面 1366 和手机 390 两种宽度共验证 19 个页面、保存 38 张截图，确认无控制台错误、无 500、无横向溢出，手机商家端重复导航已修复。
- M10 收尾、文档与部署检查已完成：README、云服务器部署手册、商家端计划和本总计划均已补齐同一套 Web 按角色分流、测试账号密码只走本地变量、MVP 单平台老板模式和未来多商家隔离边界。
- 已确认生产部署不需要第二个前端；商家端和用户端使用同一个 Web、同一个登录入口，由账号身份进入不同工作台。
- 已明确你的“从另一个中转站拿上游 API Key，再给客户二次中转”的 MVP 实现路径：你在商家端录入上游地址和上游 API Key，客户使用本平台发放的 Key 调用；多商家各自独立上游 Key 和客户归属隔离进入后续商用升级。
- 已新增 `docs/quality/t22-m10-merchant-closeout-self-check.md` 记录第 10 项自检、部署 smoke 可运行检查、密钥扫描和剩余边界。
...
- 当时未勾选 T21/T22：该账户中心补强不代表云服务器部署、DNS、HTTPS、生产 strict smoke 或上线前验收完成；后续 T22 已通过商家端独立化 M01-M10 单独完成。
