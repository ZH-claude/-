...
建议下一次从 T13 开始：分组状态页面。

T13 的边界：

- 基于当前真实模型、分组、上游状态和已有调用事件，新增分组状态页面。
- 用户只能看到自己分组可用模型的状态信息；管理员全局状态、SLA、Uptime Kuma 和复杂监控留到 T17/T20。
- 若没有足够真实统计，页面必须明确显示“暂无数据/暂无调用”，不得用假成功率或假流量填充。

T13 完成指标：

- 用户可查看自己分组可用模型的当前状态、stream 能力和近期真实调用统计或明确空状态。
- 用户可按状态筛选并刷新。
- 页面数据来自真实数据库和现有接口，不使用硬编码假统计。
- 未登录用户不能访问个人化分组状态数据。
