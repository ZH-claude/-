...
建议下一次从 T16 开始：异步任务与绘图日志。

T16 的边界：

- 基于真实上游能力和真实任务数据建 `async_tasks` 查询能力；如果当前上游不支持绘图或异步任务，页面必须显示真实空状态或未接入状态。
- 首期优先做通用异步任务表、任务查询页、用户隔离和状态同步入口；不要伪造绘图结果、进度、任务 ID 或成功状态。
- 绘图日志只在真实上游/真实任务记录存在时展示；否则保持明确空状态。
- 后续可扩展 Midjourney 类任务、图片结果、失败原因和重试，但 T16 不应破坏现有 Relay/计费/日志链路。

T16 完成指标：

- 用户只能查询自己的真实异步任务记录。
- 无任务时显示空状态；有真实任务时展示状态、平台、模型、进度、结果或失败原因。
- 普通用户不能越权查看其他用户任务。
- 浏览器验证任务页加载、筛选/刷新和空状态无控制台错误。
