...
- 基于 T09/T10 已存在的 `usage_events`、`wallet_transactions`，新增用户可查询的调用日志与消费明细。
- 用户只能看到自己的调用和账务记录，管理员后续可扩展全局筛选。
- 首期不做大范围导出，不做实时图表，只做可验证的真实表格和筛选。

T11 完成指标：

- 用户可按时间、模型、令牌、状态查询自己的调用记录。
- 消费记录能关联 request_id、usage event、wallet transaction。
- 失败调用、未知计量、成功扣费在页面和接口中状态清晰。
- 越权查询其他用户日志不可行。
