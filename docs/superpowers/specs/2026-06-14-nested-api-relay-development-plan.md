...
- 已完成本地 Chrome 真实登录验证：`merchant_test_1 / merchant200611` 在桌面 1366 宽度和手机 390 宽度下进入服务状态、请求日志和绘图日志三页，固定顶部栏和左侧栏存在，无“异步任务”误导入口、无 500、无旧英文品牌、无横向溢出，并保存截图证据。
...
- 已完成本地 Chrome 真实浏览器回归：临时普通用户真实注册登录，商家账号 `merchant_test_1 / merchant200611` 真实登录；桌面 1366 和手机 390 两种宽度共验证 19 个页面、保存 38 张截图，确认无控制台错误、无 500、无横向溢出，手机商家端重复导航已修复。
